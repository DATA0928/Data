...
public class ScoreBoardTest extends TestCase {
	...
	ILoGrades grades = new ConsLoGrades(new GradeRecord(c1, 7.5), new ConsLoGrades(new GradeRecord(c2, 5.0),
			new ConsLoGrades(new GradeRecord(c3, 7.0), new ConsLoGrades(new GradeRecord(c4, 8.0), new MTLoGrades()))));
	...
	public void testGradeAverage() {
		assertEquals(scb.gradeAverage(), 7.115, 0.001);
	}

	public void testSortByGradeDec() {
		assertEquals(l2.sortByGradeDec(), new ConsLoGrades(g1, new ConsLoGrades(g2, new MTLoGrades())));
		assertEquals(grades.sortByGradeDec(), new ConsLoGrades(g4,
				new ConsLoGrades(g1, new ConsLoGrades(g3, new ConsLoGrades(g2, new MTLoGrades())))));
	}

	public void testGreaterThanList() {
		assertEquals(empty.greaterThanList(6.0), new MTLoGrades());
		assertEquals(l2.greaterThanList(6.0), new ConsLoGrades(g1, new MTLoGrades()));
		assertEquals(grades.greaterThanList(6.0),
				new ConsLoGrades(g1, new ConsLoGrades(g3, new ConsLoGrades(g4, new MTLoGrades()))));
