...
VALUES
	...
	('KH04', 'Tran Minh Long', '50/34 Le Dai Hanh, Q10, TpHCM', '0917325476', '1965-04-09', 250000, '2006-10-02'),
	('KH05', 'Le Nhat Minh', '34 Truong Dinh, Q3, TpHCM', '08246108', '1950-03-10', 21000, '2006-10-28'),
	('KH06', 'Le Hoai Thuong', '227 Nguyen Van Cu, Q5, TpHCM', '08631738', '1981-10-31', 915000, '2006-11-24'),
	...
	('KH09', 'Le Ha Vinh', '873 Le Hong Phong, Q5, TpHCM', '08654763', '1979-09-03', 70000, '2006-01-14'),
...
VALUES
	...
	(1005, '2006-07-23', 'KH01', 'NV02', 3800000),
	...
	(1020, '2007-01-14', 'KH00', 'NV04', 70000),
	(1021, '2007-01-16', 'KH10', 'NV03', 67500),
	(1022, '2007-01-16', '', 'NV03', 7000),
	(1023, '2007-01-17', '', 'NV01', 330000);
...
--2
SELECT HOTEN AS [Nhân viên làm việc từ tháng 4/2006] 
FROM NHANVIEN WHERE MONTH(NGVL) = 4 AND YEAR(NGVL) = 2006;
--3
SELECT * FROM KHACHHANG WHERE DCHI LIKE '%Q5%';
--4
SELECT * FROM KHACHHANG
WHERE NGSINH = (SELECT MAX(NGSINH) FROM KHACHHANG);
--5
SELECT * FROM KHACHHANG
WHERE NGSINH = (SELECT MIN(NGSINH) FROM KHACHHANG);
--6
SELECT * FROM KHACHHANG
WHERE DOANHSO = (SELECT MAX(DOANHSO) FROM KHACHHANG);
--7
SELECT * FROM KHACHHANG
WHERE DOANHSO > (SELECT AVG(DOANHSO) FROM KHACHHANG);
--8
SELECT * FROM KHACHHANG
WHERE NGDK = (SELECT MIN(NGDK) FROM KHACHHANG);
--9
SELECT * FROM KHACHHANG WHERE YEAR(NGDK) = 2007;
--10
SELECT * FROM SANPHAM WHERE GIA = 3000;
--11
SELECT * FROM SANPHAM WHERE NUOCSX = 'Viet Nam';
--12
SELECT * FROM SANPHAM WHERE (TENSP LIKE 'Tap%') AND (NUOCSX = 'Trung Quoc');
--13
SELECT TENSP FROM SANPHAM WHERE (GIA >= 30000) AND (GIA <= 55000);
--14
SELECT AVG(GIA) AS [Giá trị trung bình sản phẩm do Thái Lan sản xuất] 
FROM SANPHAM WHERE NUOCSX = 'Thai Lan';
--15
SELECT H.*, C.* FROM HOADON H
...
--17
SELECT * FROM HoaDon WHERE MANV = 'N01' AND MAKH IS NULL;
--18
SELECT * FROM HOADON
WHERE MAKH = 'KH01' AND MANV = 'NV03' AND YEAR(NGHD) < 2007;
--19
SELECT COUNT(SANPHAM.DVT) AS [Số lượng hộp phấn viết bảng được bán trong năm 2006] FROM SANPHAM
INNER JOIN CTHD ON SANPHAM.MASP = CTHD.MASP
INNER JOIN HOADON ON CTHD.SOHD = HOADON.SOHD
WHERE SANPHAM.TENSP = 'Phan viet bang' AND YEAR(HOADON.NGHD) = 2006;
--20
SELECT SUM(HOADON.TRIGIA) AS [Tổng giá trị các đơn hàng của Tran Ngoc Linh] FROM HOADON
